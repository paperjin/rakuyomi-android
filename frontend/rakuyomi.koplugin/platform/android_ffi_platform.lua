...
function AndroidFFIServer:request(request)
    ...
    elseif path == "/installed-sources" then
        ...
        -- Add built-in sources that are always installed
        local has_mangadex = false
        local has_mangapill = false
        local has_weebcentral = false
        for _, src in ipairs(sources_array) do
            if src.id == "en.mangadex" then has_mangadex = true end
            if src.id == "en.mangapill" then has_mangapill = true end
            if src.id == "en.weebcentral" then has_weebcentral = true end
        end
        
        -- Hardcoded built-in sources with proper version field
        if not has_mangapill then
            table.insert(sources_array, {
                id = "en.mangapill",
                name = "MangaPill",
                lang = "en",
                source_of_source = "built-in",
        ...
        addLog(self, "Found " .. tostring(#sources_array) .. " installed sources (including built-in)")
    ...
    elseif path:match("^/jobs/download%-chapter") then
        addLog(self, "Creating download chapter job via FFI - with CBZ")
        -- Parse request body
        local body = {}
        if request.body then
    ...
    elseif path:match("^/jobs/[^/]+$") then
        ...
        -- Otherwise return pending or failed
        local job_details = {
            type = "FAILED",
            data = {
                message = "Chapter download failed"
            }
        }
        return { type = 'SUCCESS', status = 200, body = rapidjson.encode(job_details) }
